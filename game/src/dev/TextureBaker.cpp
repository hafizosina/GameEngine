...
bool TextureBaker::BakePlayer(const std::string& assetId, const std::string& outputPath) {
    LOG_INFO("Baking hero for: " + assetId);
    std::filesystem::create_directories(std::filesystem::path(outputPath).parent_path());
    
    Image img = GenImageColor(64, 64, BLANK);
    
    // Premium Hero Palette
    Color armorBody  = { 52, 73, 94, 255 };    // Midnight Blue Plate
    Color armorLight = { 93, 109, 126, 255 }; // Lighter Plate
    Color gold       = { 241, 196, 15, 255 };  // Bright Gold
    Color cape       = { 192, 57, 43, 255 };   // Royal Red
    Color shadow     = { 44, 62, 80, 255 };    // Darker Blue Shadow

    // 1. Cape (Visible from top-down perspective behind shoulders)
    ImageDrawRectangle(&img, 16, 40, 32, 8, cape);
    ImageDrawRectangle(&img, 18, 48, 28, 4, { 150, 40, 30, 255 }); // Cape shadow

    // 2. Shoulders (Simulated rounded pauldrons)
    // Left Pauldron
    ImageDrawCircle(&img, 18, 30, 14, shadow);
    ImageDrawCircle(&img, 18, 30, 12, armorBody);
    ImageDrawCircle(&img, 16, 28, 5, armorLight); // Specular highlight
    ImageDrawCircleLines(&img, 18, 30, 12, gold);

    // Right Pauldron
    ImageDrawCircle(&img, 46, 30, 14, shadow);
    ImageDrawCircle(&img, 46, 30, 12, armorBody);
    ImageDrawCircle(&img, 44, 28, 5, armorLight); // Specular highlight
    ImageDrawCircleLines(&img, 46, 30, 12, gold);

    // 3. Torso / Breastplate (Connecting the shoulders)
    ImageDrawRectangle(&img, 20, 24, 24, 18, armorBody);
    ImageDrawRectangle(&img, 20, 24, 24, 2, gold); // Collar/Trim
    
    // Chest Gem
    ImageDrawCircle(&img, 32, 34, 3, { 50, 200, 255, 255 }); // Glowing blue gem
    ImageDr#include <cmath>
#include "dev/TextureBaker.hpp"
#include "utils/Logger.hpp"
#include <raylib.h>
#include <filesystem>
...
awCircle(&img, 32, 34, 1, { 200, 255, 255, 255 });

    // 4. Head / Helmet (The central circle)
    ImageDrawCircle(&img, 32, 30, 12, shadow);    // Helmet base
    ImageDrawCircle(&img, 32, 29, 10, armorBody); // Helmet top
    ImageDrawCircle(&img, 30, 27, 4, armorLight); // Helmet highlight
    
    // Face / Visor detail
    ImageDrawRectangle(&img, 26, 26, 12, 3, { 20, 20, 25, 255 }); // Dark visor slit
    
    // 5. Hands (Gauntlets)
    ImageDrawCircle(&img, 12, 38, 6, armorBody);
    ImageDrawCircleLines(&img, 12, 38, 6, gold);
    ImageDrawCircle(&img, 52, 38, 6, armorBody);
    ImageDrawCircleLines(&img, 52, 38, 6, gold);

    // 6. Weapon (Sword held in right hand)
    Color blade = { 200, 200, 210, 255 };
    Color hilt  = { 130, 100, 40, 255 };
    ImageDrawRectangle(&img, 52, 12, 3, 24, blade);        // Blade
    ImageDrawRectangle(&img, 51, 34, 5, 2, hilt);         // Crossguard
    ImageDrawRectangle(&img, 52, 36, 3, 6, hilt);         // Handle
    ImageDrawRectangle(&img, 52, 10, 3, 2, {255,255,255,150}); // Tip highlight
